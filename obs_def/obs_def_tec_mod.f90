...
if (max_station_obs > 0) then
   ...
   if (rc /= 0) then
      write(msgstring, *) 'initial allocation failed for station TEC obs data,', &
...
if (max_limb_obs > 0) then
   ...
   if (rc /= 0) then
      write(msgstring, *) 'initial allocation failed for limb TEC obs data,', &
...
if (is_asciifile) then
   ...
   if(str1(1:7) /= 'STATION') then
      write(msgstring,*)"Expected header 'STATION' in input file, got ", str1(1:7)
...
if (is_asciifile) then
   ...
   if(str1(1:4) /= 'LIMB') then
      write(msgstring,*)"Expected header 'LIMB' in input file, got ", str1(1:4)
...
write(msgstring, *) 'teckey (',teckey,') exceeds max_station_obs (', &
                     max_station_obs,')'
